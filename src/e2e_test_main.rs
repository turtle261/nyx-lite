...
use vmm::logger::{LOGGER, LoggerConfig, error, info};
...
pub fn test_boot_shared_mem(vm: &mut NyxVM) -> u64 {
    ...
    match exit_reason {
        ExitReason::SharedMem(name, saddr, size) => {
            assert_eq!(
                name, "shared\0",
                "expected the shared memory to be registered under the name 'shared'"
            );
            assert_eq!(size, 4096, "expected to share exactly one page of memory");
            return saddr;
        }
        _ => {
            panic!("unexpected exit during boot {exit_reason:?}");
        }
    }
}
...
pub fn test_rw_shared_mem(vm: &mut NyxVM, saddr: u64) {
    ...
    let exit_reason = run_vm_test(
        vm,
        10,
        "test_rw_shared_mem: trying to read and write shared data",
    );
    match exit_reason {
        ExitReason::Hypercall(num, arg1, arg2, arg3, arg4) => {
            assert_eq!(
                num, DBG_CODE,
                "unexpected hypercall number: hypercall_dbg_code should use num {DBG_CODE:x}"
            );
            assert_eq!(arg1, 1, "expect dbgcode 1");
            assert_eq!(
                arg2, 0xb3d4f51738597a91,
                "expected the memory to be forwarded by the dbg hypercall - got {arg2:x}."
            );
    ...
    assert_eq!(
        val, 0xb3d4f51738597a91,
        "expected the guest to increment memory, got {val:x}"
    );
}
...
pub fn test_snapshot_tsc(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    for _i in 0..10 {
        ...
        let exit_reason = run_vm_test(
            vm,
            10,
            "test_snapshot_tsc: seeing how tsc responds to snapshot resets",
        );
        match exit_reason {
    ...
    for (pre, post) in pre_timestamps.iter().zip(post_timestamps.iter()) {
        assert_eq!(
            *pre, pre_timestamps[0],
            "all tsc values for pre snapshots should be the same"
        );
        assert!(
            pre <= post,
            "post snapshot tsc values should be greater or equal to pre snapshot tsc values"
        );
        if *post < last_post {
    ...
    assert!(
        !post_monotonic || post_all_equal,
        "tsc values shouldn't be increasing monotonically - this indicates the clock doesn't get set back properly"
    )
}
...
pub fn test_timeout(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let exit_reason = run_vm_test(
        vm,
        100,
        "test_timeout: make sure we can interrupt blocked vm_runs",
    );
    match exit_reason {
        ExitReason::Timeout => {
...
pub fn test_subprocess(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let exit_reason = run_vm_test(
        vm,
        500,
        "test_subprocess: ensure the guest can spawn new processes",
    );
    match exit_reason {
        ExitReason::ExecDone(code) => {
            assert_eq!(code, 23, "subprocess test should yield code 23")
        }
        _ => panic!("unexpected exit {exit_reason:?}"),
    };
}
...
pub fn test_filesystem_reset(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    for _ in 0..2 {
        ...
        let exit_reason = run_vm_test(
            vm,
            100,
            "test_subprocess: ensure that the file system is reset on snapshots",
        );
        match exit_reason {
            ExitReason::ExecDone(code) => {
                assert_eq!(code, 42, "filesystem test should yield code 42")
            }
            _ => {
...
pub fn test_single_step(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let exit_reason = run_vm_test(
        vm,
        100,
        "test_single_step: test get to known code for singlestep",
    );
...
pub fn test_host_hw_bp(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let exit_reason = run_vm_test(
        vm,
        100,
        "test_host_hw_bp: test get to known code for host bps",
    );
...
pub fn test_guest_bp(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let exit_reason = run_vm_test(
        vm,
        100,
        "test_guest_bp: ensure that the breakpoints are triggering the signal handler",
    );
    match exit_reason {
        ExitReason::Hypercall(num, arg1, arg2, _arg3, _arg4) => {
            assert_eq!(
...
pub fn test_mem_permissions(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let cr3 = vm.sregs().cr3;
    vm.vmm.lock().unwrap().set_virtual_page_permission(
        cr3,
        data + 0x1000 * 8,
        PagePermission::None,
    );
    ...
    match exit_reason {
        ExitReason::BadMemoryAccess(vec) => {
            assert_eq!(vec, [(data + 0x1000 * 8, disassembly::OpAccess::Read)])
        }
        _ => panic!("unexpected exit {exit_reason:?}"),
    };
    vm.vmm
        .lock()
        .unwrap()
        .set_virtual_page_permission(cr3, data + 0x1000 * 8, PagePermission::R);
    let exit_reason = run_vm_test(vm, 100, "test_mem_permissions: read offset");
    match exit_reason {
        ExitReason::Hypercall(num, arg1, arg2, _arg3, _arg4) => {
            assert_eq!(
    ...
    match exit_reason {
        ExitReason::BadMemoryAccess(vec) => {
            assert_eq!(vec, [(data + 0x1000 * 8, disassembly::OpAccess::Write)])
        }
        _ => panic!("unexpected exit {exit_reason:?}"),
    };
    vm.vmm
        .lock()
        .unwrap()
        .set_virtual_page_permission(cr3, data + 0x1000 * 8, PagePermission::RW);
    let exit_reason = run_vm_test(vm, 100, "test_mem_permissions: write offset");
    match exit_reason {
        ExitReason::Hypercall(num, arg1, arg2, _arg3, _arg4) => {
            assert_eq!(
...
pub fn test_incremental_snapshot(
    vm: &mut NyxVM,
    shared_vaddr: u64,
    base_snapshot: &Arc<NyxSnapshot>,
) {
...
pub fn test_shutdown(vm: &mut NyxVM, shared_vaddr: u64, snapshot: &Arc<NyxSnapshot>) {
    ...
    let exit_reason = run_vm_test(
        vm,
        100,
        "test_shutdown: ensure that the vm can respond to shutdown cleanly",
    );
    match exit_reason {
        ExitReason::Shutdown => {}
        _ => panic!("unexpected exit {exit_reason:?}"),
